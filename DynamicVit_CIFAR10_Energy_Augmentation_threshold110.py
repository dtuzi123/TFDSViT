...
'''
def Attention(input, name, is_training,batch_size=64, reuse=False):
    with tf.variable_scope(name) as scope:
        if reuse:
            scope.reuse_variables()
...
def Attention(input, name, is_training,batch_size=64, reuse=False):
    z_dim = 100
    #z_mean = linear(input, z_dim, 'e_mean')
    z_mean = linear(input, z_dim, name)
    return z_mean
#
def Pre_DNN(input, name, is_training,batch_size=64, reuse=False):
    with tf.variable_scope(name) as scope:
        if reuse:
            scope.reuse_variables()

        z_dim = 100
        z_out = lrelu(bn(linear(input, z_dim, scope='c_fc5'), is_training=is_training, scope='c_bn5'))
    return z_out
...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.expansionThreshold = 19.95

        self.featuresArr = []
        self.recoErrArr = []

    def Sparse_Classifier(self,name, image, mask, reuse=False):
        with tf.compat.v1.variable_scope(name) as scope:
            if reuse:
                scope.reuse_variables()

            batch_size = 64
            image2 = tf.reshape(image, (batch_size, 28, 28, 1))
            patch_size = 7
            patches = tf2.image.extract_patches(
                images=image2,
            ...
            # patch_dim = 16
            patch_count = 16
            patches = tf.reshape(patches, [64, -1, patch_dim])
            # (64,16,192)
            ...
            is_training = True
            features = Pre_DNN(patches, "bbb", is_training,
                          batch_size=64, reuse=False)

            myArr = []
            for j in range(self.headDim):
                str1 = "encoder_V" + str(j)
                str2 = "encoder_K" + str(j)
                str3 = "encoder_Q" + str(j)

                is_training = True
                V =  Attention(features, str1, is_training,
                                                              batch_size=64, reuse=False)
                K =  Attention(features, str2, is_training,
                                                              batch_size=64, reuse=False)
                Q =  Attention(features, str3, is_training,
                                                              batch_size=64, reuse=False)
                ...
                arr = output
                arr = tf.reshape(arr,(self.batch_size,-1))
                arr = arr * mask[j]

                if np.shape(myArr)[0] == 0:
                    myArr = arr
                else:
                    myArr = tf.concat((myArr,arr),axis=1)

            arr = myArr

            output1 = lrelu(bn(linear(arr, 200, scope='c_fc5'), is_training=is_training, scope='c_bn5'))
            self.featuresArr.append(output1)

            continous_len = 10
            logoutput = linear(output1, continous_len, 'e_log_sigma_sq')

            return logoutput

    def Build_subModel(self,arr,scopName):

        is_training = True
        with tf.compat.v1.variable_scope(scopName) as scope:
            output1 = lrelu(bn(linear(arr, 500, scope='c_fc5'), is_training=is_training, scope='c_bn5'))
            output1 = lrelu(bn(linear(output1, 200, scope='c_fc6'), is_training=is_training, scope='c_bn6'))
    ...
    def Build_SelfAttention(self,name,embeddingName, image, mask, reuse=False):
        with tf.compat.v1.variable_scope(name) as scope:
            if reuse:
                scope.reuse_variables()
            ...
            # (64,16,192)
            patches = tf.reshape(patches, (-1, 192))

            is_training = True

            #features = Pre_DNN(patches, embeddingName, is_training, batch_size=64, reuse=reuse)

            z_dim = 100
            features = lrelu(bn(linear(patches, z_dim, scope=embeddingName), is_training=is_training, scope=embeddingName+str(1000)))

            self.headDim = 6

            tAttentionArr = self.attentionArr
            myArr = []
            for j in range(self.headDim):
                str1 = "encoder_V" + str(j)
                str2 = "encoder_K" + str(j)
    ...
    def Build_Weight(self,name,tAttentionArr):
        with tf.compat.v1.variable_scope(name) as scope:
            weights = tf.Variable(
                tf.random_normal(shape=[np.shape(tAttentionArr)[0]], mean=0, stddev=1),
                name='a1', trainable=True)
            #weights = tf.nn.softmax(weights)
            weights = self.gumbel_softmax(weights,0.5)

        return weights

    def build_multi(self,weights,tAttentionArr,arr):
        totalArr = []
        expertIndexCount = int(np.shape(tAttentionArr)[0] / 6)
        for expertIndex in range(expertIndexCount):
            arr2 = []
            startIndex = expertIndex * 6
            for d in range(6):
                aa2 = tAttentionArr[startIndex + d]
                aa2 = aa2 * weights[startIndex + d]

                if np.shape(arr2)[0] == 0:
                    arr2 = aa2
                else:
                    arr2 = tf.concat((arr2, aa2), axis=1)
            if np.shape(totalArr)[0] == 0:
                totalArr = arr2
            else:
                totalArr = totalArr + arr2

            # arr = arr + arr2
        #
        arr = tf.concat((arr, totalArr), axis=1)
        return arr

    def Sparse_Classifier2(self,name, image, mask, reuse=False):
        with tf.compat.v1.variable_scope(name) as scope:
            if reuse:
                scope.reuse_variables()
            ...
            features = Pre_DNN(patches, name, is_training, batch_size=64, reuse=reuse)
            ...
            name = "subClassifier" + str(self.componentCount)

            logoutput = self.Build_subModel(arr,name)
        return logoutput
    ...
    def Classifier(self,name, image, z_dim=20, reuse=False):
        with tf.compat.v1.variable_scope(name) as scope:
            ...
            features = Pre_DNN(patches, name, is_training, batch_size=64, reuse=False)
            ...
            if self.componentCount > 1:
                weights = tf.Variable(tf.random_normal(shape=[np.shape(self.featuresArr)[0] + 1], mean=0, stddev=1),
                                      name='a1')
                weights = tf.nn.softmax(weights)
                output1 = lrelu(bn(linear(arr, 500, scope='c_fc5'), is_training=is_training, scope='c_bn5'))
                output1 = lrelu(bn(linear(output1, 200, scope='c_fc6'), is_training=is_training, scope='c_bn6'))

                outSum = weights[0] * output1
                for j in range(np.shape(self.featuresArr)[0]):
                    out1 = self.featuresArr[j]
                    out1 = out1 * weights[j + 1]
                    outSum = outSum + out1

                self.featuresArr.append(output1)

                continous_len = 10
                logoutput = linear(outSum, continous_len, 'e_log_sigma_sq')
    ...
    def Create_VAEs(self,index,selectedComponentIndex):
        ...
        attentionIndex = 0
        maskValue = np.zeros(6)
        #maskValue = np.ones(6)
        aa = (self.componentCount + 1) % 2
        buildFirst = 0
        if aa == 1:
            buildFirst = 1
            maskValue[3:6] = 1
        ...
        expertParameters = "Expert" + str(self.expertIndex)
        weightParameters = "Weight" + str(self.componentCount)
        classifierParameters = "classifier" + str(self.componentCount)
        embeddingParameters = "Embedding" + str(self.expertIndex)
        if aa == 0:
            self.expertIndex = self.expertIndex+1

        # Classifier
        if buildFirst == 1:
            tAttentionArr, arr = self.Build_SelfAttention(expertParameters,embeddingParameters, self.inputs, maskValue, reuse=False)

            weights = self.Build_Weight(weightParameters,tAttentionArr)

            arr = self.build_multi(weights,tAttentionArr, arr)

            logits = self.Build_subModel(arr,classifierParameters)
        else:
            tAttentionArr, arr = self.Build_SelfAttention(expertParameters,embeddingParameters, self.inputs, maskValue, reuse=True)
        ...
        self.recoErrArr.append(reconstruction_loss1)
        ...
        expertParameters_ = [var for var in T_vars if var.name.startswith(expertParameters)]
        weightParameters_ = [var for var in T_vars if var.name.startswith(weightParameters)]
        classifierParameters_ = [var for var in T_vars if var.name.startswith(classifierParameters)]
        embeddingParameters_ = [var for var in T_vars if var.name.startswith(embeddingParameters)]

        myclassifier =  weightParameters_ + expertParameters_ + classifierParameters_# + embeddingParameters_
    ...
    def my_gumbel_softmax_sample(self,logits, cats_range, temperature=0.1):
    ...
    def Predictions_By_Index2(self,testX,index):
        ...
        for i in range(myN):
            my1 = testX[self.batch_size * i:self.batch_size * (i + 1)]
            myPrediction = self.TeacherPredictions[minIndex]
            predictions = self.sess.run(myPrediction, feed_dict={self.inputs: my1})
            for k in range(self.batch_size):
                totalPredictions.append(predictions[k])
        totalPredictions = np.array(totalPredictions)
    ...
    def Calculate_Accuracy_ByALL_Batch(self, testX,testY, index3):
        totalCount = np.shape(testX)[0]
        myPro = []
    ...
    def Computer_Distance(self,code1,code2):
        count1 = np.shape(code1)[0]
        sum1 = 0
        for i in range(count1):
            op1=np.sqrt(np.sum(np.square(code1[i]-code2[i])))
            sum1 = sum1 + op1
        sum1 = sum1 / count1
        return sum1

    def Calculate_Eneory_Memory(self):
        count = np.shape(self.FixedMemory)[0] - 64
        bahtchX = self.FixedMemory[count -64: count]
        arr = []
        for j in range(np.shape(self.recoErrArr)[0] - 1):
            sum1 = 0
            batch1 = bahtchX
            loss = self.sess.run(self.recoErrArr[j],feed_dict={self.inputs:batch1})
            arr.append(loss)
        return arr

        '''
        count = int(np.shape(self.FixedMemory)[0] / self.batch_size)
        arr = []
        for j in range(np.shape(self.recoErrArr)[0]):
            sum1 = 0
            for i in range(count):
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            expansionCount = int(np.shape(self.totalSet)[0] / self.data_stream_batch)
            expansionCount = int(expansionCount / 8.0)
            ...
            for t1 in range(int(np.shape(self.totalSet)[0] / self.data_stream_batch)):
                ...
                #if t1 == expansionCount:
                '''
                if self.keyCount > 1000:
                    self.keyCount = 0
                    self.Create_VAEs(self.componentCount + 1, 0)
                    self.FixedMemory = []
                ...
                if self.componentCount == 1:
                    ...
                    #trainData = train_datagen.fit(self.FixedMemory,self.FixedMemoryLabel)
                    #train_gen = train_datagen.flow(self.FixedMemory, self.FixedMemoryLabel, batch_size=64)

                    ucount = 0
                    umaxCount = 200
                    for x_batch,y_batch in train_datagen.flow(self.FixedMemory,self.FixedMemoryLabel, batch_size=64, shuffle=True):
                        ucount = ucount +1

                        if ucount > umaxCount:
                            break

                        if np.shape(x_batch)[0] == 64:
                            batchImages = x_batch
                            batchLabels = y_batch
                            ...
                            _, _, d_loss = self.sess.run([self.TeacherOptimArr[self.componentCount - 2],
                                                          self.TeacherOptimArr[self.componentCount - 1], self.vaeLoss],
                                                         feed_dict={self.inputs: batchImages, self.label: batchLabels,
                                                                    self.z: batch_z})

                            _, _ = self.sess.run([self.ClassifierOptimArr[self.componentCount - 2],
                                                  self.ClassifierOptimArr[self.componentCount - 1]],
                                                 feed_dict={self.inputs: batchImages, self.label: batchLabels,
                                                            self.z: batch_z})
                    ...
                    ucount = 0
                    umaxCount = 200
                    for x_batch, y_batch in train_datagen.flow(self.FixedMemory, self.FixedMemoryLabel, batch_size=64,
                                                               shuffle=True):
                        ucount = ucount + 1
                        ...
                        if np.shape(x_batch)[0] == 64:
                            ...
                            _, d_loss,_ = self.sess.run([self.TeacherOptimArr[self.componentCount-1], self.vaeLoss,self.ClassifierOptimArr[self.componentCount-1]],
                                                      feed_dict={self.inputs: batchImages,self.label:batchLabels
                                                                 })
                ...
                if np.shape(self.FixedMemory)[0] > self.maxMmeorySize:
                    ...
                    if self.componentCount == 1:
                        ...
                        scores = self.Calculate_Eneory_Memory()
            ...
            print(int(self.componentCount/2))
            ...
            myThirdName = "results/TFDSViT_CIFAR10_RealN" + str(self.threshold)+".txt"
            ...
            myThirdName = "results/TFDSViT_CIFAR10_Component" + str(self.threshold)+".txt"
            f = open(myThirdName, "w", encoding="utf-8")
            for i in range(np.shape(arr1)[0]):
                f.writelines(arr1[i])
                f.writelines('\n')
