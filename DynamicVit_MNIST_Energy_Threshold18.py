...
def Pre_DNN(input, name, is_training,batch_size=64, reuse=False):
    with tf.variable_scope(name) as scope:
        ...
        z_dim = 100
        output1 = lrelu(bn(linear(input, z_dim, scope='c_fc5'), is_training=is_training, scope='c_bn5'))
        return output1

def AuxilaryBenchmark_MNIST_Encoder_Small_a_Fixed(input, name, is_training,batch_size=64, reuse=False):
    with tf.variable_scope(name) as scope:
        if reuse:
            scope.reuse_variables()
...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.epoch = 30
        self.classifierLearnRate = 1e-4
        self.expansionThreshold = 9.4
        #
    ...
    def Classifier(self,name, image, z_dim=20, reuse=False):
        with tf.compat.v1.variable_scope(name) as scope:
            ...
            if self.componentCount > 1:
                ...
                weights = tf.Variable(tf.random_normal(shape=[np.shape(self.featuresArr)[0] + 1], mean=0, stddev=1), name='a1',trainable=True)
                weights = tf.nn.softmax(weights)

                #outSum = weights[0] * arr
    ...
    def Create_Student(self,index,selectedComponentIndex):
        ...
        # Classifier
        logits = self.Classifier(classifierName, self.inputs, self.z_dim, reuse=False)
    ...
    def Create_VAEs(self,index,selectedComponentIndex):
        ...
        if attentionIndex == 0:
            classifierName = "Classifier" + str(index)
            logits = self.Sparse_Classifier(classifierName, self.inputs, maskValue, reuse=False)
        else:
            classifierName = "Classifier" + str(index-1)
            logits = self.Sparse_Classifier(classifierName, self.inputs, maskValue, reuse=True)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            print(int(np.shape(self.GeneratorArr)[0] / 2.0))

            print("N of paramaters")
            totalN = self.count_param()
            print(totalN)

            for i in range(np.shape(self.GeneratorArr)[0]):
                batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
                gan1 = self.sess.run(self.GeneratorArr[i],feed_dict={self.z:batch_z})
                gan1 = np.reshape(gan1,(-1,28,28,1))
                gan1 = gan1 * 255.0
                str2 = "LEViT_MNIST" + str(i) + ".png"
            ...
            myThirdName = "results/DTransformer_MNIST_Final.txt"
            ...
            myThirdName = "results/DTransformer_MNIST_Final_Components_940.txt"
            f = open(myThirdName, "w", encoding="utf-8")
            for i in range(np.shape(arr1)[0]):
                f.writelines(arr1[i])
                f.writelines('\n')
