...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.parameterArr = []
        self.label = tf.placeholder(tf.float32, [self.batch_size, 100])
        self.KDlabel = tf.placeholder(tf.float32, [self.batch_size, 100])
    ...
    def Create_VAEs(self,index,selectedComponentIndex):
        ...
        if attentionIndex == 0:
            classifierName = "Classifier" + str(index)
            logits = self.Sparse_Classifier2(classifierName, self.inputs, maskValue, reuse=False)
        else:
            classifierName = "Classifier" + str(index - 1)
            logits = self.Sparse_Classifier2(classifierName, self.inputs, maskValue, reuse=True)
    ...
    def Calculate_Acc(self,test,testLabel):
        ...
        for i in range(count):
            ...
            prediction = self.TeacherPredictions[index] # self.classifierModelArr[index].predict(batch)
            #prediction = np.argmax(prediction,axis=1)
            prediction = self.sess.run(prediction, feed_dict={self.inputs:batch})
                ...
                if self.componentCount == 1:
                    ...
                    for x_batch, y_batch in train_datagen.flow(self.FixedMemory, self.FixedMemoryLabel, batch_size=64,
                        ...
                        if np.shape(x_batch)[0] == 64:
                            ...
                            _, d_loss, _ = self.sess.run([self.TeacherOptimArr[self.componentCount - 1], self.vaeLoss,
                                                          self.ClassifierOptimArr[self.componentCount - 1]],
                                                         feed_dict={self.inputs: batchImages, self.label: batchLabels
                                                                    })
